...
// 에러 메시지 표시 - 사용자가 고칠 때까지 화면에 남는다
function showError(message) {
    clearTimeout(messageTimer);          // 앞선 자동 초기화 예약을 취소한다
    formError.textContent = message;
    formError.style.display = 'block';
    formError.style.color = '#dc3545';   // 빨간색
}
...
function showSuccess(message) {
    ...
    formError.textContent = message;
    formError.style.display = 'block';
    formError.style.color = '#28a745';   // 초록색
 
    // 3초 뒤에 clearMessages 를 실행하라고 예약해 둔다
...
function validateStudent(student) {
  ...
  if (!student.studentNumber || !isValidStudentNumber(student.studentNumber)) {
    alert("학번을 입력하지 않거나 올바른 학번 형식이 아닙니다.");
    return false;
  }
